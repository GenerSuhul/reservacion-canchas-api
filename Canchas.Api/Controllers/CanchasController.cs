...
namespace Canchas.Api.Controllers
{
    ...
    public class CanchasController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] bool? activo)
        {
            var consulta = _context.Canchas.AsNoTracking().AsQueryable();
        ...
        [HttpPost]
        public async Task<IActionResult> Post(CanchaDto dto)
        {
            var nombre = dto.Nombre.Trim();

            if (await _context.Canchas.AnyAsync(c => c.Nombre == nombre))
                return BadRequest(new { mensaje = "Ya existe una cancha con ese nombre." });
        ...
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Put(int id, CanchaDto dto)
        {
            ...
            if (await _context.Canchas.AnyAsync(c =>
                c.Nombre == nombre &&
                c.IdCancha != id))
            {
            ...
            return Ok(new { mensaje = "Cancha actualizada correctamente." });
        }

        [HttpPut("{id:int}/estado")]
        public async Task<IActionResult> Estado(int id, EstadoDto dto)
        {
            var cancha = await _context.Canchas.FindAsync(id);
            ...
            return Ok(new { mensaje = "Estado de la cancha actualizado." });
        }
    }
}
